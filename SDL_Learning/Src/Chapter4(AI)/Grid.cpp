#include "Grid.h"
#include<algorithm>
#include"Application4.h"
#include"Tower.h"
#include"Enemy.h"

namespace SDLChapter4
{
	void Grid::SelectTile(size_t row, size_t col)
	{
		//make sure it;s a valid selection
		Tile::TileState tstate = mTiles[row][col]->GetTileState();
		if (tstate != Tile::TileState::EStart && tstate != Tile::TileState::EBase)
		{
			if (mSelectedTile)
			{
				mSelectedTile->ToggleSelect();
			}

			mSelectedTile = mTiles[row][col];
			mSelectedTile->ToggleSelect();
		}
	}

	void Grid::UpdatePathTiles(Tile* start)
	{
		//Reset all tiles to normal (except start/end)
		for (size_t i = 0; i < NumRows; i++)
		{
			for (size_t j = 0; j < NumCols; j++)
			{
				if (!(i == 3 && j == 0) && !(i == 3 && j == 15))
				{
					mTiles[i][j]->SetTileState(Tile::TileState::EDefault);
				}
			}
		}

		Tile* t = start->mParent;
		while (t != GetEndTile())
		{
			t->SetTileState(Tile::TileState::EPath);
			t = t->mParent;
		}
	}

	Grid::Grid(Application4* game)
		:Actor{ game },mSelectedTile{nullptr}
	{
		mTiles.resize(NumRows);
		for (size_t i = 0; i < mTiles.size(); i++)
		{
			mTiles[i].resize(NumCols);
		}

		// Create tiles
		for (size_t i = 0; i < NumRows; i++)
		{
			for (size_t j = 0; j < NumCols; j++)
			{
				mTiles[i][j] = new Tile(static_cast<Application4*>(GetGame()));
				mTiles[i][j]->SetPosition(Vector2(TileSize / 2.0f + j * TileSize, StartY + i * TileSize));
			}
		}

		//set start/end tiles
		GetStartTile()->SetTileState(Tile::TileState::EStart);
		GetEndTile()->SetTileState(Tile::TileState::EBase);

		//set up adjancey lists
		for (size_t i = 0; i < NumRows; i++)
		{
			for (size_t j = 0; j < NumCols; j++)
			{
				if (i > 0)
				{
					mTiles[i][j]->mAdjacent.push_back(mTiles[i - 1][j]);
				}

				if (i < NumRows - 1)
				{
					mTiles[i][j]->mAdjacent.push_back(mTiles[i + 1][j]);
				}

				if (j > 0)
				{
					mTiles[i][j]->mAdjacent.push_back(mTiles[i][j - 1]);
				}

				if (j < NumCols - 1)
				{
					mTiles[i][j]->mAdjacent.push_back(mTiles[i][j + 1]);
				}
			}
		}

		//find path(in reverse)
		FindPath(GetEndTile(), GetStartTile());
		UpdatePathTiles(GetStartTile());

		mNextEnemy = EnemyTime;
	}

	void Grid::ProcessClick(int x, int y)
	{
		y -= static_cast<int>(StartY - TileSize / 2);
		if (y >= 0)
		{
			x /= static_cast<int>(TileSize);
			y /= static_cast<int>(TileSize);

			if (x >= 0 && x < static_cast<int>(NumCols) && y >= 0 && y < static_cast<int>(NumRows))
			{
				SelectTile(y, x);
			}
		}
	}

	bool Grid::FindPath(Tile* start, Tile* goal)
	{
		for (size_t i = 0; i < NumRows; i++)
		{
			for (size_t j = 0; j < NumCols; j++)
			{
				mTiles[i][j]->g = 0.0f;
				mTiles[i][j]->mInOpenSet = false;
				mTiles[i][j]->mInClosedSet = false;
			}
		}

		std::vector<Tile*> openSet{};

		Tile* current = start;
		current->mInClosedSet = true;

		do
		{
			for (Tile* neighbor : current->mAdjacent)
			{
				if (neighbor->mBlocked)
					continue;

				if (!neighbor->mInClosedSet)
				{
					if (!neighbor->mInOpenSet)
					{
						neighbor->mParent = current;
						//heuristic function
						neighbor->h = (neighbor->GetPosition() - goal->GetPosition()).Length();
						//path cost
						neighbor->g = current->g + TileSize;
						//f(x) = g(x) +h(x)
						neighbor->f = neighbor->g + neighbor->h;

						openSet.emplace_back(neighbor);
						neighbor->mInOpenSet = true;
					}
					else
					{
						float newG = current->g + TileSize;
						if (neighbor->g > newG)
						{
							neighbor->mParent = current;
							neighbor->g = newG;

							neighbor->f = neighbor->g + neighbor->h;
						}
					}
				}
			}

			if (openSet.empty())
				break;

			//find the lowest in openset and make them current
			auto iter = std::min_element(openSet.begin(), openSet.end(),
				[](Tile* a, Tile* b) {
					return a->f < b->f;
				});

			current = *iter;
			openSet.erase(iter);
			current->mInOpenSet = false;
			current->mInClosedSet = true;

		} while (current != goal);

		return current == goal;
	}

	void Grid::BuildTower()
	{
		if (mSelectedTile && !mSelectedTile->mBlocked)
		{
			mSelectedTile->mBlocked = true;
			if (FindPath(GetEndTile(), GetStartTile()))
			{
				Tower* t = new Tower(static_cast<Application4*>(GetGame()));
				t->SetPosition(mSelectedTile->GetPosition());
			}
			else
			{
				// This tower would block the path, so don't allow build
				mSelectedTile->mBlocked = false;
				FindPath(GetEndTile(), GetStartTile());
			}
			UpdatePathTiles(GetStartTile());
		}
	}

	Tile* Grid::GetStartTile()
	{
		return mTiles[3][0];
	}

	Tile* Grid::GetEndTile()
	{
		return mTiles[3][15];
	}

	void SDLChapter4::Grid::UpdateActor(float deltaTime)
	{
		Actor::UpdateActor(deltaTime);

		mNextEnemy -= deltaTime;
		if (mNextEnemy <= 0.0f)
		{
			new Enemy(static_cast<Application4*>(GetGame()));
			mNextEnemy += EnemyTime;
		}
	}
}
